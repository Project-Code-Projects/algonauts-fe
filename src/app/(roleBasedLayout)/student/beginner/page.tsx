...
const styles = {
  ...
  chapterCard:
    " glassmorphism shadow-md rounded-lg p-6 hover:shadow-lg transition duration-300",
...
const BeginnerPage = () => {
  ...
  const advancedSection = allSectionsData?.data.find(
    (section: ISection) => section.name === "beginner"
  );
  const beginnerSectionId = advancedSection ? advancedSection._id : null;

  const {
    data: getAllChaptersFromBE,
    isLoading: ChaptersLoading,
    isError: ChaptersLoadingError,
  } = useGetSectionByIdQuery(beginnerSectionId);
  ...
  return (
    <div className={styles.container}>
      <h1 className={styles.title}>Beginner Section</h1>
      <div className={styles.chaptersContainer}>
        {getAllChaptersFromBE?.data?.chapters?.map((chapter: IChapter) => (
          <Link
            href={`/student/beginner/chapter/${chapter._id}`}
            key={chapter._id}
